...
5. **🔌 Комплексный сканер API (REST / GraphQL / Swagger)**
   - Система осуществляет поиск и анализ различных типов программных интерфейсов, учитывая, что бизнес-логика современных приложений часто реализуется именно через API.
   - **REST API:** Поиск стандартных и скрытых маршрутов (например, `/api/v1/`, `/api/users`). Проводятся базовые проверки на уязвимости контроля доступа (Broken Object Level Authorization - BOLA/IDOR), выявляя конечные точки, которые отдают чувствительные JSON-данные без токенов авторизации.
   - **GraphQL API:** Обнаружение GraphQL-эндпоинтов (`/graphql`) и проверка на критическую уязвимость — **включенную интроспекцию (Introspection)**. Сканер тестирует, может ли злоумышленник запросить `__schema` и получить полную структуру данных сервера.
   - **Swagger / OpenAPI:** Поиск публично доступной документации (Swagger UI, `openapi.json`), случайно оставленной в production-среде. Открытая документация раскрывает векторы атак, показывая все доступные методы и параметры.
   - *Архитектурное решение:* Сканер не просто перебирает пути, но и отправляет тестовые `POST`/`GET` запросы, анализируя `Content-Type` и структуру ответов (JSON), чтобы подтвердить наличие уязвимости.
